...
public interface SubscribedUserRepository extends PagingAndSortingRepository<SubscribedUser, String> {
    ...
    @Query("SELECT u FROM SubscribedUser u WHERE u.zipCode >= :zipcode ORDER BY u.promotionFactor DESC, u.userMstTypeId DESC, u.zipCode ASC")
    Page<SubscribedUser> suggestionsForRetailersByZipCode(@Param("zipcode") int zipcode, Pageable pageable);

    @Query("SELECT u FROM SubscribedUser u WHERE u.zipCode >= :zipcode ORDER BY u.promotionFactor DESC, u.userMstTypeId ASC, u.zipCode ASC")
    Page<SubscribedUser> suggestionsForSuppliersByZipCode(@Param("zipcode") int zipcode, Pageable pageable);

    @Query("SELECT u FROM SubscribedUser u WHERE u.zipCode >= :zipcode ORDER BY u.promotionFactor DESC, u.userMstTypeId ASC, u.zipCode ASC")
    Page<SubscribedUser> suggestionsForCompanyByZipCode(@Param("zipcode") int zipcode, Pageable pageable);
